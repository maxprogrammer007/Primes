...
{
    public class PoolQ2MSieve : ISieveRunner
    {
        public string Name => "poolq2m";
        public string Description => "Array pool, 64 bit, 1 of 2, bitmasking";
        ...
        public PoolQ2MSieve(int sieveSize)
        {
            if (sieveSize < 0)
        ...
        public void Run()
        {
            ...
            for (int factor = 3; factor <= q; factor += 2)
            {
                ...
                for (int bitCount = 0, m = factor * factor; bitCount < dataBits; bitCount++, m += (factor << 1))
                {
                    masks[iUsed] |= MaskOf(m);

                    offset = IndexOf(m + (factor << 1)) - IndexOf(m);
